...
import {UceSubject} from '../models/uce';
...
export class UceComponent implements OnInit {
  ...
  electives: UceSubject[];
  uceSubjects: UceSubject[];
  compulsorySubjects: UceSubject[];
  ...
  electiveChanged(element: any, e: { target: { value: any; }; }) {
    ...
    let elementValue = e.target.value;
    const elementNameGrade = elementName + 'Grade';
    ...
    //     const elementParentName = elementName.substring(0, 12);
  ...
  loadSubjects(): void {
    this.mainService
        .getUceSubjects()
        .subscribe((data: UceSubject[]) => {
          this.uceSubjects = data;
